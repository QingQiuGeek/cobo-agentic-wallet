...
export default function ServiceGrid({ onPurchase }: ServiceGridProps) {
  const [services, setServices] = useState<Service[]>(servicesCache?.services || []);
    ...
    <div className="flex flex-col h-full">
      ...
      {source && !error && (
        <div className="px-3 py-1 text-[9px] text-zinc-400 border-b border-zinc-100 dark:border-zinc-900 shrink-0 flex items-center justify-between">
          <span>Source: Bazaar Discovery</span>
          {cacheRemaining > 0 && (
            <span className="opacity-60">
              Cache: {Math.floor(cacheRemaining / 60)}:{String(cacheRemaining % 60).padStart(2, '0')}
            </span>
          )}
        </div>
      )}
      ...
      {error && (
        <div className="px-3 py-3 flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-800 shrink-0">
      ...
      >
        <div className="flex flex-col gap-2 p-2">
          {services.map((service) => (
            <ServiceCard key={service.id} service={service} onPurchase={onPurchase} />
          ))}
        </div>
      </PaginatedList>
    </div>
