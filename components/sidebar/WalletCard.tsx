...
export default function WalletCard({ onWalletSwitch }: WalletCardProps) {
	...
	const getTokenDisplayInfo = (tokenId: string) => {
		const symbol = tokenId.includes('USDC')
			? 'USDC'
			: tokenId.includes('USDT')
				? 'USDT'
				: tokenId.includes('ETH')
					? 'ETH'
					: tokenId.includes('SOL')
						? 'SOL'
						: tokenId.split('_').pop() || tokenId;
		const isTestnet =
			tokenId.startsWith('S') ||
			tokenId.startsWith('T') ||
			tokenId.startsWith('SOLDEV');
	...
	return (
		<div
			...
			className='border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 bg-white dark:bg-zinc-950 shadow-sm transition-all'
		>
			{/* Header */}
			<div
				id='wallet-trigger'
			...
			{isOpen && (
				<div
					...
					className='pt-2 border-t border-zinc-100 dark:border-zinc-900 flex flex-col gap-3'
				>
					{!isConnected ? (
						/* Not connected */
						...
						<>
							...
							{currentWallet?.evmAddress && (
								<div className='flex items-center justify-between py-1 bg-zinc-50 dark:bg-zinc-900/40 rounded px-2'>
									<span className='text-xs text-zinc-400'>EVM</span>
								...
								<div className='grid grid-cols-2 gap-2'>
									{balances.slice(0, 6).map((b, idx) => {
										...
										return (
											<div
												key={idx}
												className='bg-zinc-50 dark:bg-zinc-900/40 rounded p-2 border border-zinc-100 dark:border-zinc-900'
											>
												<div className='flex items-center justify-between'>
													...
													{isTestnet && (
														<span className='text-[8px] bg-amber-500/10 text-amber-600 dark:text-amber-400 px-1 rounded'>
															测试网
														</span>
													)}
												</div>
